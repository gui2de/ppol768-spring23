...
program define part_one1, rclass 
...
program define part_one2, rclass 
	syntax, samplesize(integer)
	use "part_one_file", clear 
	sample `samplesize', count
...
program define part_one3, rclass 
	...
	use "part_one_file", clear 
	sample `samplesize', count
	reg emission treatment i.state activity 
		mat a = r(table)
		return scalar beta = a[1,1]
		return scalar pval = a[4,1]
		return scalar N = a[7,1]+2 
...
capture program drop part_one4
program define part_one4, rclass 
	syntax, samplesize(integer)
	use "part_one_file", clear 
	sample `samplesize', count
...
capture program drop part_one5
program define part_one5, rclass 
	syntax, samplesize(integer)
	use "part_one_file", clear 
	sample `samplesize', count
...
forvalues i=100(100)500{
	forvalues j=1(1)5{
	tempfile sims
	simulate beta=r(beta), reps(300) seed(1234) saving(`sims') : part_one`j', samplesize(`i')
	use `sims' , clear
	gen samplesize=`i'
	gen model = `j'
