...
expand 3+int((159-3+1) *rnormal()) //generate county-leveldataset. Range of 3 to 159 counties based on actual data of US states (Delaware has least no. of counties with 3 and Georgia has most with 159)
bysort state: generate county = _n
gen county_effects = rnormal(0, 2) //county effects

*generate X = DACA approvals
gen daca = 10 + int((35-10+1)*runiform()) //create a variable that measures DACA approvals (in thousands). For purposes of this simulation, states have between 10,000 to 35,000 approvals in a uniform distribution

*3. gen different covariates 
gen bluestate = runiform()
gen mean_years_school = rnormal(14, 2) //mean years of schooling at state level. Confounder.
...
gen unemployment = 3.6 // avg. unemployment rate in US
...
reg unemployment daca bluestate
reg unemployment daca bluesetate mean_years_school 
reg unemployment daca bluesetate mean_years_school healthspend_per_capita 
reg unemployment daca bluesetate mean_years_school healthspend_per_capita non_english_household
reg unemployment daca bluesetate mean_years_school healthspend_per_capita non_english_household i.state //state fixed effects added
