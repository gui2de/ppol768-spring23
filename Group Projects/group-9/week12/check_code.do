...
drop accper typrep b001100000 b002000000 b006000000 rptdate rptmonth Revenue NetProfit OCI f090701b EPS lag_EPS a001000000 a002000000 a003000000 Liabilities Assets short_name TRD_YearYsmvttl FS_CombasTyprep FS_CombasA003000000 FS_CominsTyprep FS_CominsB001100000 BookValue MktValue

//Due to github's 25MB data upload limit, we split the 138MB dashboard file into 4 separate excel files.
*Check missing data
...
export excel using "Dashboard_Missing Data", sheet("Missing Data", modify) firstrow(variables)
// 16 missing data

*Check duplicated ID and Time(year)
duplicates tag stkcd rptyr, gen(duplicate)
count if duplicate!= 0
preserve
drop missing_data
export excel using "Dashboard_Duplicated Data", sheet("Duplicate", modify) firstrow(variables)
restore
// No duplicate data

global dashboard "Dashboard_Outlier.xlsx"
...
export excel using "Dashboard_Outlier", sheet("Outlier_lag revenue", modify) firstrow(variables)
restore
...
export excel using "Dashboard_Outlier", sheet("Outlier_SalesGrouth", modify) firstrow(variables)
restore
...
export excel using "Dashboard_Outlier", sheet("Outlier_Change in EPS", modify) firstrow(variables)
restore
...
export excel using "Dashboard_Outlier", sheet("Outlier_Debt to Assets", modify) firstrow(variables)
restore
...
export excel using "Dashboard_Outlier", sheet("Outlier_Book to Market", modify) firstrow(variables)
restore
...
export excel using "Final dataset", sheet("Final Dataset", modify) firstrow(variables)
